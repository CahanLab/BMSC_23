...
# Import the file
# Please download the GSE234540_BMSC_day3 data from GEO before start
...
# AnnData object with n_obs × n_vars = 3208 × 223398

#Make variables unique
adata_gem.var_names_make_unique()

# AnnData object with n_obs × n_vars = 3208 × 32285
...
# View of AnnData object with n_obs × n_vars = 2870 × 19176
...
# View of AnnData object with n_obs × n_vars = 2870 × 19065
...
sc.pp.neighbors(adata_gem, n_neighbors=20,n_pcs=30)
...
#Keep cluster 0, 1, 6, and 8

# Save the File
adata_gem.write_loom('Day3_pre_clustered_GEX_112122.loom', write_obsm_varm=True)
adata_all.write_loom('Day3_clustered_GEX_112122.loom', write_obsm_varm=True)

a= adata_all[adata_all.obs['leiden'].isin(['0', '1', '6', '8'])].obs.index
# 1762 cells

# Multi-tag assignment
download.file("s3://cahanlab/ray.cheng/BMSC_2023/BMSC_Multipotency/tag_assignment_final_053121.txt", "tag_assignment_final_053121.txt")
assignment = pd.read_csv("tag_assignment_final_053121.txt", sep = '\t', header = 0)
assignment.index = assignment.index.astype(str) + '-1'
adata_all.obs['multitag'] = assignment['multitag']

b = adata_all[adata_all.obs['multitag'].isin(['Bar3'])].obs.index
...
adata_gem= adata_gem[adata_gem.obs.index.isin(b)]
adata_gem
...
# View of AnnData object with n_obs × n_vars = 1687 × 17932
...
# Save the File
adata_gem.write_loom('Day3_pre_clustered_GEX_cleaned_112122.loom', write_obsm_varm=True)
adata_all.write_loom('Day3_clustered_GEX_cleaned_112122.loom', write_obsm_varm=True)

adata_all.obs_names= adata_all.obs.index.str[:-2]
adata_all.obs_names.to_frame().to_csv('Day3_filtered_cells_20221122.txt', header=False, index=False)


# ATAC Part:
adata_atac = adata_atac[adata_all.obs_names, :]

# View of AnnData object with n_obs × n_vars = 1687 × 191113
# Add annotation
download.file("s3://cahanlab/ray.cheng/BMSC_2023/BMSC_Multipotency/GSE234540_BMSC_day3_atac_peak_annotation.tsv", "atac_peak_annotation.tsv")
...
# AnnData object with n_obs × n_vars = 1637 × 190495
...
min_score_value = 0.535
nb_feature_selected = 125000
...
adata_all.write_loom('Day3_clustered_GEX_ATAC_filtered_112122.loom', write_obsm_varm=True)
adata_atac_raw.write_loom("Day3_ATAC_112122.loom", write_obsm_varm=True)
adata_atac_all.write_loom("Day3_clustered_ATAC_112122.loom", write_obsm_varm=True)

# we eventually obtain 1637 high quality nuclei
