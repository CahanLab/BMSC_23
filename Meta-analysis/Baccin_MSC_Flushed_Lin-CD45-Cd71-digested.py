...
adata=  sc.read("/Users/raycheng/Desktop/Baccin_MSC/GSM3937217/outs/velocyto/GSM3937217_VRRW0.loom", cache=True)
adata
...
#Cluster 2: Osteoblast
#Cluster 3: B cell
#Cluster 4: SMC

# subsetting
a= adata_all[adata_all.obs['louvain'].isin(['0', '2', '4'])].obs.index
adata=  sc.read("GSM3937217_VRRW0.loom", cache=True)
adata
...
adata_all.obs["gene_over_count"] = adata_all.obs["n_genes"]/adata_all.obs["n_counts"]
sc.pl.umap(adata_all, color = 'gene_over_count')


#Finding Marker Genes that consider all the genes (I keep the embedding the same)
sc.tl.rank_genes_groups(adata_all, 'louvain')
a = pd.DataFrame(adata_all.uns['rank_genes_groups']['names']).head(50)

pd.DataFrame(adata_all.uns['rank_genes_groups']['pvals']).head(25)

# Remove nonrelated cells
adata_all = sc.read("Baccin_BMSC4_extracted_clustered.loom")
a= adata_all[adata_all.obs['louvain'].isin(['0', '1', '2'])].obs.index
adata=  sc.read("GSM3937217_VRRW0.loom")
adata
